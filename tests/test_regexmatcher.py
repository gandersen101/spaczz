...
def test_add_with_more_patterns_than_explicit_kwargs_warns() -> None:
...
def test_add_with_more_explicit_kwargs_than_patterns_warns() -> None:
...
def test_add_without_string_pattern_raises_error() -> None:
...
def test_add_str_pattern_outside_list_raises_error() -> None:
...
def test_add_where_kwargs_are_not_dicts_raises_error() -> None:
...
def test_len_returns_count_of_labels_in_matcher() -> None:
...
def test_in_returns_bool_of_label_in_matcher() -> None:
...
def test_labels_returns_label_names() -> None:
...
def test_remove_label_raises_error_if_label_not_in_matcher() -> None:
    """It raises a ValueError if trying to remove a label not present."""
    with pytest.raises(ValueError):
        rm.remove("TEST")


def test_matcher_returns_matches() -> None:
...
def test_patterns_with_and_without_kwargs() -> None:
    """It adds the "GPE" label and a pattern with some kwargs to the matcher."""
    temp_rm = RegexMatcher(nlp.vocab)
    temp_rm.add("GPE", ["[U](nited|\\.?) ?[S](tates|\\.?)"])
...
def test_matcher_pipe_with_context() -> None:
...
def test_matcher_pipe_with_matches() -> None:
...
def test_matcher_pipe_with_matches_and_context() -> None:
