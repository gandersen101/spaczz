...
- *The `flex` parameter now defaults to `max(len(pattern) - 1, 0)` instead of `len(pattern)` as this generally makes more sense. Single-token patterns shouldn't have their boundaries extended during optimization by default.*
